  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      ...
      >
        ...
        <span>
          Auto-publish via <code style={{ background: "var(--inline-code-bg)", padding: "1px 6px", borderRadius: 4 }}>/api/cron/publish</code> ·
          Requires <code style={{ background: "var(--inline-code-bg)", padding: "1px 6px", borderRadius: 4 }}>CRON_SECRET</code> in env
        </span>
      </div>
      ...
      {posts.length === 0 ? (
        <div className="omg-card" style={{ padding: "56px 24px", textAlign: "center" }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13 }}>Approve posts and use &quot;Schedule&quot; to set a publish time.</p>
        </div>
      ) : (
                  ...
                  >
                    ...
                    {p.scheduledAt && (
                      <div style={{ textAlign: "right" }}>
                        ...
                        <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{new Date(p.scheduledAt).toLocaleDateString()}</div>
                      </div>
                    )}
                  </div>
                ))}
