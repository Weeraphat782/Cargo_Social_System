...
export default async function DashboardPage() {
  ...
  const upcomingPayload = upcomingPosts.map((p) => {
  ...
  const runsPayload = recentRuns.map((r) => ({
    id: r.id,
  ...
  return (
    <DashboardClient
      initialStats={{ pending, approved, scheduled, published, topics: topicCount }}
      initialTopics={topics}
      activeCampaignTotal={activeCampaignTotal}
      initialCampaigns={campaignPayload}
      initialUpcoming={upcomingPayload}
      initialRecentRuns={runsPayload}
    />
  );
}
