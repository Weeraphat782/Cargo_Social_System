...
export default function CampaignsClient({ initialCampaigns }: { initialCampaigns: CampaignRow[] }) {
  ...
  function openAiConfirm(s: SuggestedCampaign) {
    ...
    setAiDraft({
      ...
      contentLanguage: manualForm.contentLanguage,
      campaignGoal: "",
      targetPersona: "",
      contentPillars: "",
      keywords: s.keywords,
    ...
    try {
      const res = await fetch("/api/campaigns", {
        ...
        body: JSON.stringify({
          ...
          campaignGoal: draft.campaignGoal.trim() || undefined,
          targetPersona: draft.targetPersona.trim() || undefined,
          contentPillars: draft.contentPillars.trim() || undefined,
