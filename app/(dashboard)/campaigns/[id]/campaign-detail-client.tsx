...
export default function CampaignDetailClient({ initialData }: { initialData: CampaignDetailData }) {
  const router = useRouter();
  const id = initialData.id;
  const [c, setC] = useState<CampaignDetailData>(initialData);
  ...
  return (
    <div>
      ...
      >
        ...
        {c.nextRunAt != null ? (
          <CountdownRing targetIso={new Date(c.nextRunAt).toISOString()} label="Next run" size={56} />
        ) : null}
      </div>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderColor: active ? "var(--border)" : "var(--border)", // Keep consistent border
