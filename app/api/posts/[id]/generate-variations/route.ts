...
export async function POST(
  ...
  try {
    const candidates = await Promise.all(
      Array.from({ length: count }, (_, idx) =>
        generateAndUploadImage({
          ...
          storageKeyPrefix: `posts/${postId}/${platformSlug}/variations/${ts}-${idx}-${Math.random().toString(36).slice(2, 10)}`,
    ...
    return NextResponse.json({
      ok: true,
      candidates: candidates.map((c) => ({ imageUrl: c.imageUrl, prompt: c.prompt })),
    });
  } catch (err) {
    ...
    console.error("[generate-variations] failed:", raw);
