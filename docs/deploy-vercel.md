...
This runbook matches `vercel.json` in the repo: two daily crons and `maxDuration: 60` for long serverless routes.
...
| `/api/cron/publish` | `0 0 * * *` | ~07:00 |

Vercel Cron sends the `x-vercel-cron` header; the app also accepts manual calls with `CRON_SECRET` (see `lib/cron-auth.ts`).
...
- **Meta app (OAuth UI):** `META_APP_ID`, `META_APP_SECRET` (see `.env.example`).
- **Facebook Page + Instagram Business:** Page access token, Page ID, and IG user ID are configured in the app under **Connections** (stored encrypted in the database), not as long-term production env vars — see `README.md` “Meta”.
...
4. In the app UI, confirm: campaign **next run** / upcoming posts / **Recent runs** and `/logs` (PublishLog) as applicable.
